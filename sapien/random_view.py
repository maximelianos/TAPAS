...
def main():
    ...
    urdf_path = 'objects/3971/mobility.urdf'
    ...
    for n_episode in range(10):
        ...
        x = r * np.cos(a)
        y = r * np.sin(a)
        z = np.random.rand() * (max_r-min_r) + min_r
        
        cam_pos = np.array([x, y, z]) # np.array([-2, -2, 3])
        ...
        # x_0 = C x_1, x_0 - woorld coors, x_1 - camera coors
        x_1 = np.array([0, 0, 0, 1])
        x_0 = mat44 @ x_1
        assert np.allclose(x_0[:3], cam_pos)
        ...
        rgba_pil.save(save_dir / f'{n_episode:04d}_color.png')
        ...
        depth_pil.save(save_dir / f'{n_episode:04d}_depth.png')
        
        # JSON annotations
        annotations.append(
            {
                "episode": n_episode,
                "extrinsics": mat44.tolist()
            }
        )
    
    # JSON annotations
    with open(save_dir / "intrinsics.json", "w") as f:
        json.dump(camera.get_intrinsic_matrix().tolist(), f, indent=4, ensure_ascii=False)

    with open(save_dir / "annotations.json", "w") as f:
        json.dump(annotations, f, indent=4, ensure_ascii=False)


    depth_valid = depth[position[..., 3] < 1]  # exclude invalid pixels
    min_depth = float(depth_valid.min())
    max_depth = float(depth_valid.max())

    print("Depth min (m):", min_depth)
    print("Depth max (m):", max_depth)

    # Write to file
    with open("depth_stats.txt", "w") as f:
        f.write(f"Depth min (m): {min_depth:.4f}\n")
        f.write(f"Depth max (m): {max_depth:.4f}\n")
