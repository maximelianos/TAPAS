...
def annotate_masks(
    ...
    for idx, mask in enumerate(masks):
        ...
        # Draw the mask
        if draw_mask:
            mask_rgb = np.zeros_like(annotated)
            mask_rgb[mask > 0] = color
            annotated = cv2.addWeighted(annotated, 1.0, mask_rgb, alpha, 0)

        # Find bounding box
        if draw_box or draw_mark:
            ...
            if contours:
                ...
                if draw_mark:
                    text = str(idx + 1) if label_mode == "1" else f"obj_{idx + 1}"
                    tx, ty = (x, y - 5) if mark_position == 'top_left' else (x + w // 2, y + h // 2)
                    cv2.putText(annotated, text, (tx, ty), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
