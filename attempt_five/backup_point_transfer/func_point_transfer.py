...
import torch
from PIL import Image
from my_models.keypoint_encoder import KeypointEncoder ## this needs to be trained for function points as well as grasp point
...
_TAPAS  = KeypointEncoder().to(_DEVICE).eval()

_IMG_H, _IMG_W = 480, 640         # what the ViT cfg expects
_TRANSFORM = T.Compose([
    T.Resize((_IMG_H, _IMG_W)),
    T.ToTensor(),
])
...
def get_features_local(image: Image.Image) -> np.ndarray:
    img_tensor = transform(image).unsqueeze(0).to(dino_model.device)
    with torch.no_grad():
...
def func_point_transfer(data_path, test_data_path, test_tool_label='mug', test_target_label='bowl', task_label='pour', num_candidate_keypoints=8, sd_dino_flag=False):
    ...
    ## Be careful especially in this part.




    # ---------- TAPAS-based coarse key-point proposal ----------------
    ##x_kp_crop, y_kp_crop, kp_idx, info, _ = _tapas_single_salient_point(test_init_frame_pil_crop)
    
    

    #crop_w, crop_h = test_init_frame_pil_crop.size          # (width, height)
    #x_kp_crop = int(round(x_kp_crop * crop_w / 640))
    #y_kp_crop = int(round(y_kp_crop * crop_h / 480))


    # --- prepare all 2-D keypoints for visualisation -----------------
    kp_2d = info["kp_raw_2d"]
    if isinstance(kp_2d, tuple):
        kp_2d = kp_2d[0]

    vec       = kp_2d.squeeze(0).cpu().numpy()      # shape (2 × N,)
    num_kp    = vec.shape[0] // 2
    u_norm    = vec[:num_kp]                        # first  N entries  → x
    v_norm    = vec[num_kp:]                        # second N entries → y
    kp_norm   = np.stack([u_norm, v_norm], axis=1)  # shape (N,2), values in [-1,1]

    u_norm, v_norm = kp_norm[kp_idx]          # values in [-1, 1]
    ...
    kp_px = (kp_norm + 1) / 2
    kp_px[:, 0] *= crop_w          # real crop width
    kp_px[:, 1] *= crop_h          # real crop height

    print(f"[DEBUG] blue-dot (crop px): ({x_kp_crop}, {y_kp_crop})   "
      f"crop size: ({crop_w}, {crop_h})")
    # -----------------------------------------------------------------

    # --------- draw every keypoint (red) and the selected one (blue) -
    debug_vis = np.array(test_init_frame_pil_crop)[..., ::-1].copy()  # BGR
    for u, v in kp_px.astype(int):
        cv2.circle(debug_vis, (u, v), 3, (0, 0, 255), -1)             # red
    cv2.circle(debug_vis, (x_kp_crop, y_kp_crop), 6, (255, 0, 0), -1) # blue
    cv2.imwrite(os.path.join(output_path, "visual_prompting_result.png"),
                debug_vis)
    # -----------------------------------------------------------------

    # remap to full test-frame coordinates
    x_offset, y_offset, _, _ = test_tool_box
    vp_func_point = (x_kp_crop + x_offset, y_kp_crop + y_offset)

    test_init_frame_x = int(round(vp_func_point[0]))
    test_init_frame_y = int(round(vp_func_point[1]))
    ...
    # Get dimensions of the full test image
    frame_h, frame_w = test_init_frame.shape[:2]

    # Normalize coordinates
    x_norm = test_init_frame_x / frame_w
    y_norm = test_init_frame_y / frame_h

    frame_h, frame_w = test_init_frame.shape[:2]
    x_px = int(round(x_norm * frame_w))
    y_px = int(round(y_norm * frame_h))


    # Visualize (still use pixel coords for visualization)

    cv2.circle(test_init_frame, (x_px, y_px), 3, (0, 0, 255), -1)
    cv2.imwrite(os.path.join(output_path, 'test_init_frame_func_point_vis.jpg'), test_init_frame)

    print(f"[DEBUG] Drawing point at normalized: ({x_norm:.4f}, {y_norm:.4f}) → pixel: ({x_px}, {y_px})")

    ##print(f"[Pixel]   Intended function point: ({test_init_frame_x}, {test_init_frame_y})")
    print(f"[Normalized] Function point: ({x_norm:.4f}, {y_norm:.4f})")

    # Save normalized output
    with open(os.path.join(output_path, 'test_init_frame_func_point_out.json'), "w") as json_file:
        json.dump([x_norm, y_norm], json_file, indent=4)
