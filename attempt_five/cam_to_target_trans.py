...
def demo_test_detection(
        data_path: str,
        tool_label: str,
        target_label: str,
        *,                       # everything after this is keyword-only
        owl_params: dict | None = None,
        owl: OWLViT | None = None,
        sam_predictor: SAM | None = None,
):
    """
    detect tools and targets in demo and test scenes
    """
    if owl is None:
        owl = OWLViT(**(owl_params or {}))   #  ← reuse the params block
...
def cam_to_target_trans(data_path):
    ...
    initial_frame_index = 0
    initial_frame_filename = f'{initial_frame_index:05d}.jpg'
    initial_frame_file_path = os.path.join(data_path, 'rgb', initial_frame_filename)
    ...
    # Load or create depth image
    if os.path.exists(initial_frame_depth_file_path):
        initial_frame_depth_pil = Image.open(initial_frame_depth_file_path)
        initial_frame_depth = np.array(initial_frame_depth_pil)
    else:
        print("failed to detect depth map")
        height, width = initial_frame.shape[:2]
        initial_frame_depth = np.zeros((height, width), dtype=np.uint16) 
    ...
    print("Depth min/max:", initial_frame_depth.min(), initial_frame_depth.max())
    ...
    if target_mask.shape != initial_frame.shape[:2]:
        raise ValueError(f"Mask shape {target_mask.shape} does not match image shape {initial_frame.shape[:2]}")

    # Check how many pixels will be kept
    mask_coverage = np.count_nonzero(target_mask)
    if mask_coverage < 100:
        raise ValueError(f"Target mask is too sparse ({mask_coverage} pixels). Cannot reconstruct object.")
    ...
    # Convert mask to boolean: True where the target object exists
    mask_bool = target_mask_uint8.astype(bool)

    # Apply the mask to the RGB image
    masked_initial_frame = np.where(mask_bool[..., None], initial_frame, 0)

    # Apply the mask to the depth map safely (no bitwise on float32)
    masked_initial_frame_depth = np.where(mask_bool, initial_frame_depth, 0.0).astype(np.float32)
    points, colors, points_3d = compute_point_cloud(masked_initial_frame, masked_initial_frame_depth, CAM_MATRIX, [])
    print("Points generated from masked image:", points.shape[0])
    target_point_cloud = o3d.geometry.PointCloud()
    
    print("Shape of `points`:", points.shape)
    print("Dtype of `points`:", points.dtype)
    print("First few points:\n", points[:5])


    if points.size == 0:
        raise RuntimeError("Target point cloud is empty. Check mask or depth image quality.")
